...
def draw_time_space_diagram(direction, filename, sa_num=None, end_time=1800):
    ...
    for veh_id in range(1, end_time + 1, 10):
        t = veh_id
        log = []
        first_row = filtered_all.iloc[0]
        curr_pos = first_row["cumulative_distance"] if first_row["distance_from_prev_meter"] > 0 else 0  # 시작 위치
        ...
        for _, row in filtered_all.iterrows():
            ...
            travel_time = dist / speed if dist > 0 else 0
            ...
            if not green_ok.empty:
                # ✅ 이동 구간 추가 (curr_pos → next_pos)
                log.append({
                    "vehicle_id": veh_id,
                    "time": round(arrival, 1),
                    "position": round(next_pos, 1),
                ...
                future_green = green_df[
                    (green_df["intersection_name"] == intersec) &
                    (green_df["green_start_time"] >= arrival - 5)  # 여유 5초 허용
                ...
                wait_start = round(arrival, 1)
                wait_end = round(future_green.iloc[0]["green_start_time"], 1)
