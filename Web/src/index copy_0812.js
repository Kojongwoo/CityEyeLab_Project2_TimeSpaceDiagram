...
// 궤적 이동 전역 변수
let selectedTrajectoryIndex = -1; // 선택된 궤적의 인덱스. -1은 선택되지 않음을 의미합니다.
let isMoving = false;             // 현재 궤적을 이동 중인지 여부
let dragStartPoint = null;        // 드래그 시작 시점의 마우스 좌표
...
deleteDrawnToggle.addEventListener("change", (e) => {
  ...
  deleteDrawnLabel.style.color = isDeleteDrawnMode ? "#2e7d32" : "#888";
});
...
canvas.addEventListener("mousedown", (e) => {
  ...
  // 그리기 모드가 아닐 때 (궤적 선택 및 이동 로직)
  // 1. 클릭한 위치에 있는 궤적 찾기
  const clickedIndex = drawnTrajectories.findIndex(traj => 
    pointToLineDistance(coords.x, coords.y, traj.start.x, traj.start.y, traj.end.x, traj.end.y) < 5 // 5px 근처를 클릭했는지 확인
  );

  // 2. 이미 선택된 궤적을 다시 클릭하면 '이동 모드' 시작
  if (clickedIndex !== -1 && clickedIndex === selectedTrajectoryIndex) {
    isMoving = true;
    dragStartPoint = coords;
  } 
  // 3. 새로운 궤적을 클릭하면 '선택' 상태로 변경
  else {
    selectedTrajectoryIndex = clickedIndex;
    isMoving = false; // 이동은 아직 시작하지 않음
  }
  
  redrawCanvas(); // 선택 상태가 변경되었으므로 캔버스를 다시 그림
});
...
canvas.addEventListener("mousemove", (e) => {
  if (isDrawMode && isDrawing) {
    ...
    const rect = canvas.getBoundingClientRect();
    currentLinePreviewEnd = getCanvasCoords(e);
  ...
  // ▼ 이동 모드일 때의 로직을 새로 추가합니다.
  if (isMoving && selectedTrajectoryIndex !== -1) {
    const coords = getCanvasCoords(e);
    const dx = coords.x - dragStartPoint.x; // X축 이동량
    const dy = coords.y - dragStartPoint.y; // Y축 이동량

    // 선택된 궤적의 시작점과 끝점 좌표를 이동량만큼 업데이트
    ...
    // 속도, 각도 등 파생 데이터도 다시 계산
    const t0 = pxToTime(trajectory.start.x);
    const p0 = pxToPos(trajectory.start.y);
    const t1 = pxToTime(trajectory.end.x);
    const p1 = pxToPos(trajectory.end.y);
    
    const dt = t1 - t0;
    const dp = p1 - p0;
    trajectory.vMps = dt !== 0 ? dp / dt : 0;
    trajectory.vKph = trajectory.vMps * 3.6;
    let angle = Math.atan2(dp, dt) * 180 / Math.PI;
    trajectory.angleDeg = angle < 0 ? angle + 360 : angle;


    dragStartPoint = coords; // 현재 마우스 위치를 새로운 시작점으로 갱신
    redrawCanvas();
  }
});
...
function redrawCanvas() {
    ...
    drawnTrajectories.forEach((traj, index) => {
      ...
      if (index === selectedTrajectoryIndex) {
        ctx.strokeStyle = "#e91e63"; // 선택됐을 때 핑크색
        ctx.lineWidth = 4;           // 더 굵게
      } else {
        ctx.strokeStyle = "#ff9800"; // 기본 주황색
        ctx.lineWidth = 2;
      }
